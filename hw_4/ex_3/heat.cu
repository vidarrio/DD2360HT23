...
int main(int argc, char **argv) {
  ...
  cusparseDnVecDescr_t xDescriptor;  // Vector descriptor needed by cuSPARSE
  cusparseDnVecDescr_t yDescriptor;  // Vector descriptor needed by cuSPARSE
  ...
  //@@ Insert code to call cusparse api to create the vector descriptor used by cuSPARSE
  cusparseCheck(cusparseCreateDnVec(&xDescriptor, dimX, temp, CUDA_R_64F));
  ...
  cusparseCheck(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, Adescriptor, xDescriptor, &zero, yDescriptor, CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, &bufferSize));
  ...
  for (int it = 0; it < nsteps; ++it) {
    ...
    //@@ tmp = 1 * A * temp + 0 * tmp
    cusparseCheck(cusparseSpMV(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, Adescriptor, xDescriptor, &zero, yDescriptor, CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, buffer));
  ...
  cusparseCheck(cusparseDestroyDnVec(xDescriptor));
